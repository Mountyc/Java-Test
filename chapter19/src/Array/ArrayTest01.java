...
Array
    ...
    6、数组一日创建，在java中规定，长度不可变。
    7、数组的分类一维数组、二维数组、三维数组、多维数组...(一维数组较多，二维数组偶尔使用!）
    ...
    10、数组在内存方面储的时候，数组中的素内存地址(存储的每一个元素都是有规则的排列的)是连续的。内存地址连续
    ...
    12、数组这种数据结构的优点和缺点是什么?
        优点:查询/查找/检索某个下标上的元素时效率极高。可以说是查询效率最高的一个数据结构。
            为什么检索效率高?
                ...
                第二:每一个壳素类型相同，所以占用空间大小一样。
                第三:知道第一个元素内存地址，知道每一个元素占用空间的大小，又知道下标，所以
                通过一个数学表达式就可以计算出某个下标上元素的内存地址。
     ...
     14、怎么声明定义一个一维数组?
           语法格式:
            ...
            String[] names =new String[]://初始化6个长度的String类型数组，每个元素默认值null.
 */
public class ArrayTest01 {
    public static void main(String[] args) {
        //声明一个int类型的数组，使用静态初始化的方式
         int[] a = {1,100,10,20,55,600};
         //这是c++风格，不建议java使用
        //int a [] =  {1,100,10,20,55,600};
        ...
        //下标为6表示第七个元素，第七个元素没有，下标越界了会出现什么异常
        //System.out.println(a[6]);//ArrayIndexOutOfBoundsException(比较著名异常)
